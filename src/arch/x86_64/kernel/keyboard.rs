...
pub(crate) fn get_keyboard_handler() -> (u8, fn()) {
	unsafe {
		...
		// 1. Flush the keyboard buffer (remove leftover BIOS data)
		while (cmd_port.read() & 0x01) != 0 {
			let _ = data_port.read();
		}

		// 2. Read PS/2 controller configuration (Command 0x20)
		cmd_port.write(0x20);
		let mut config = data_port.read();

		// 3. Enable hardware interrupts for the keyboard (Bit 0)
		config |= 0b0000_0001;

		// 4. Write new configuration back to the controller (Command 0x60)
		cmd_port.write(0x60);
